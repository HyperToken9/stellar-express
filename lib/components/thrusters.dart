...
class Thrusters extends Component with HasGameRef<StarRoutes> {
  ...
  late Map<String, List<Thruster>> thrusterMap =
  {
    ...
    "Cargo Ship": <Thruster>[
      /* Forward */
      Thruster(position: Vector2(-150, 0), direction: Vector2(-1, 0), thrust: 20,
              colors: [const Color(0x6FFFFFFF), const Color(0x6F979797)],
              triggers: {"forward"}, thrusterSize: 50),
      // /* Cold Gas */
      // Thruster(position: Vector2(-300, -500), direction: Vector2(0, -2), thrust: 12,
      //     colors: [const Color(0x6F94B0C2), const Color(0x6FF4F4F4)],
      //     triggers: {"backward", "left"}, thrusterSize: 60, priority: 5),
  ...
  Thrusters({required this.attachedTo});
  ...
  void forwardThrusters(double thrustFraction){
    for (Thruster thruster in thrusters){
      // print("Forward Thrusters");
      if (thruster.triggers.contains("forward")){
        // print("Adding Particles");
        _addParticles(thruster, thrustFraction);
      }
    }
  }

  void backwardThrusters(double thrustFraction){
    for (Thruster thruster in thrusters){
  ...
  void leftThrusters(double thrustFraction){
    for (Thruster thruster in thrusters){
  ...
  void rightThrusters(double thrustFraction){
    for (Thruster thruster in thrusters){
  ...
  void _addParticles(Thruster thruster, [double thrustFraction = 1]){
    final List<Particle> particles = thruster.colors.map((color) {
      return createAcceleratedParticle(
        ...
        size: Vector2.all(thruster.thrusterSize) * attachedTo.scale.x,
        color: color,
        direction: _rotateVector(thruster.direction)
                  * thruster.thrust * thrustFraction,
        thrust: thruster.thrust * thrustFraction,
      );
    }).toList();
    ...
    gameRef.world.add(
      ParticleSystemComponent(
        ...
        priority: max(1, attachedTo.priority - 1),
  ...
  Particle createAcceleratedParticle({
    ...
    final double velocityFactor = thrust / 16;
  ...
  Vector2 _calculatePosition(Vector2 offset) {
    // offset = offset / attachedTo.scale.x;
    final double angle = attachedTo.angle;
...
class Thruster{
  ...
  double thrust;
  double thrusterSize;
  /*TODO : MAKE THIS A PRIORITY OFFSET */
  int priority;
